...
@interface XDCalendarPicker ()<UITableViewDataSource, UITableViewDelegate, XDRefreshTableViewDelegate>
{
    CGPoint _previousOffSet; //tableView上一次开始滑动的偏移量，用以判断是上划还是下划
    
    XDCalendarCenter *_calendarCenter;
    
    //手势，style==XDCalendarStyleOneWeek时使用
...
- (id)initWithPoint:(CGPoint)point calendarStyle:(XDCalendarStyle)style selectedDate:(NSDate *)date
{
    ...
    if (self) {
        ...
        _calendarCenter = [XDCalendarCenter defaultCenter];
        _calendarCenter.activityDate = date;
...
- (void)selectedFromBlock:(NSNotification *)notification
{
    ...
    if ([object isKindOfClass:[XDDay class]])
    {
        ...
        _calendarCenter.activityDate = selectedDay.blockDate;
        _calendarCenter.activityDay = selectedDay;
    }
}
...
- (void)removeAllGestureRecognizer
{
    ...
    [self.view removeGestureRecognizer:_downRecognizer];
}
...
//    self.tableView.userInteractionEnabled = NO;
    ...
    if (gestureRecognizer.state == UIGestureRecognizerStateEnded) {
        ...
        if (indexPath.row < ([self.tableView numberOfRowsInSection:0] - 1)) {
            [self scrollFromIndexPath:indexPath toIndexPath:[NSIndexPath indexPathForRow:(indexPath.row + 1) inSection:indexPath.section]];
        }
    }
}
...
//    }
    ...
    if (gestureRecognizer.state == UIGestureRecognizerStateEnded) {
        if (_delegate && [_delegate respondsToSelector:@selector(calendarPickerSwipeDown:)]) {
            [_delegate calendarPickerSwipeDown:self];
        }
    }
}
...
- (void)setSelectedDate:(NSDate *)date
{
    if (![_selectedDate isEqualToDate:[date convertToZeroInMorning]]) {
        ...
        if (_delegate && [_delegate respondsToSelector:@selector(calendarPicker:selectedDate:)]) {
            [_delegate calendarPicker:self selectedDate:_calendarCenter.activityDay.blockDate];
        }
    }
}
...
- (void)refreshDots
{
    if (_delegate && [_delegate respondsToSelector:@selector(calendarPickerDayDotViews:)]) {
        _calendarCenter.dotViews.dictionary = [_delegate calendarPickerDayDotViews:self];
...
- (void)scrollFromIndexPath:(NSIndexPath *)fromIndex toIndexPath:(NSIndexPath *)toIndex
{
    CGRect rect = self.tableView.frame;
    XDWeekCell *fromCell = (XDWeekCell *)[self.tableView cellForRowAtIndexPath:fromIndex];
    
    UIImageView *currentWeekImgView = [[UIImageView alloc] initWithImage: [self imageFromView:fromCell]];
    currentWeekImgView.frame = CGRectMake(0, rect.origin.y, rect.size.width, rect.size.height);
    [self.view addSubview: currentWeekImgView];
    
    CGFloat originX1 = 0;
    CGFloat originX2 = 0;
    BOOL isSwipeRight = fromIndex.row > toIndex.row ? YES : NO;
    if (isSwipeRight) {
        originX1 = -320;
        originX2 = 321;
    }
    ...
    self.tableView.frame = CGRectMake(originX1, rect.origin.y, 320, rect.size.height);
    ...
    [UIView animateWithDuration: .65
                     animations:^{
                         currentWeekImgView.frame = CGRectMake(originX2, rect.origin.y, rect.size.width, rect.size.height);
                         self.tableView.frame = CGRectMake(0, rect.origin.y, rect.size.width, rect.size.height);
                     }
...
- (void)activityDate:(NSDate *)date withRequest:(BOOL)isRequest
{
    ...
    if (day) {
        _calendarCenter.activityDay = day;
        day.selected = YES;
        _calendarCenter.activityDate = day.blockDate;
    }
...
{
    if (_isScrolling == YES) {
        [self stopScrollingAtOffset:self.tableView.contentOffset];
    }
}
